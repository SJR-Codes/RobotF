...
#    Create Session    OYF    ${url}     auth=${auth}
    ...
    ${response}=    POST On Session    OYF  url=${login_url}    data=${body}    headers=${heads}
...
#    Create Session    OYF    ${url}     auth=${auth}
    ...
    ${response}=    POST On Session    OYFUSER  url=${login_url}    data=${body}    headers=${heads}
    Should Be Equal As Strings      ${response.status_code}     200
#    Log  Response is ${response.json()['access_token']}  console=yes
    ${token}=    Set Variable    ${response.json()['access_token']}
    ${local_headers}    Create Dictionary    Content-Type=application/json    Authorization=Bearer ${token}
...
Is User Authorized
    ${resp}=  GET On Session  OYFUSER  ${user_url}me    headers=${user_headers}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()['is_active']}  True
    Should Be Equal As Strings  ${resp.json()['is_superuser']}  False

Can User Access OYF
    ${resp}=  GET On Session  OYFUSER  ${oyf_base_url}    headers=${user_headers}
    Should Be Equal As Strings  ${resp.status_code}  200
    Should Be Equal As Strings  ${resp.json()['message'][:5]}  Hello
Can User Access OYF Categories
    ${resp}=  GET On Session  OYF  ${oyf_categories_url}    headers=${user_headers}
    Should Be Equal As Strings  ${resp.status_code}  200
    #Should Be Equal As Strings  ${resp.json()['message'][:5]}  Hello

Delete User
    ${resp}=  DELETE On Session  OYF  url=${user_url}${uid}    headers=${super_headers}
